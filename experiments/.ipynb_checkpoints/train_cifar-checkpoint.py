...
import models
sys.path.append(os.path.join(MAIN_PATH,'pnormsoftmax'))
...
def gain(logits,labels,metric = metrics.AUROC_min):
    risk = measures.wrong_class(logits,labels)
    p,beta = pNormSoftmax.optimize.p_and_beta(logits,risk,metric)
    return metric(risk,measures.MSP(logits))-metric(risk,pNormSoftmax.pNormSoftmax(logits,p,beta))
...
risk_dict = {'accuracy': metrics.accuracy,
             'loss': torch.nn.CrossEntropyLoss(),
                ...
                #'p_star': lambda x,y: pNormSoftmax.optimize.p(x,measures.wrong_class(x,y),metrics.AUROC_min),
                #'T_star': lambda x,y: pNormSoftmax.optimize.T(x,measures.wrong_class(x,y),metrics.AUROC_min),
                #'AUROC_gain': gain
                }
